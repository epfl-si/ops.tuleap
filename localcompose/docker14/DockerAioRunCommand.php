...
{
    // private const TULEAP_FQDN = getenv('TULEAP_FQDN') === FALSE ? 'tuleap.local' : (string) getenv('TULEAP_FQDN');
    // private const MYSQL_HOST = getenv('MYSQL_HOST') === FALSE ? '' : getenv('MYSQL_HOST');
    // private const MYSQL_USER = getenv('MYSQL_USER') === FALSE ? '' : getenv('MYSQL_USER');
    // private const MYSQL_PASSWORD = getenv('MYSQL_PASSWORD') === FALSE ? '' : getenv('MYSQL_PASSWORD');
    ...
    {
        $tuleap_fqdn = getenv('TULEAP_FQDN') === FALSE ? 'tuleap.local' : (string) getenv('TULEAP_FQDN');
        try {
    ...
    private function installTuleap(OutputInterface $output, Tuleap $tuleap): void
    {
        ...
        $tuleap_fqdn = getenv('TULEAP_FQDN') === FALSE ? 'tuleap.local' : (string) getenv('TULEAP_FQDN');
        $mysql_host = getenv('MYSQL_HOST') === FALSE ? '' : getenv('MYSQL_HOST');
        $mysql_user = getenv('MYSQL_USER') === FALSE ? '' : getenv('MYSQL_USER');
        $mysql_password = getenv('MYSQL_PASSWORD') === FALSE ? '' : getenv('MYSQL_PASSWORD');

        $mysql_daemon = $this->initializeMysqlDataStore($output);
        $ssh_daemon->startDaemon($output);
        $tuleap->setup($output, $tuleap_fqdn, $mysql_host, $mysql_user, $mysql_password);
        $this->shutdownMysql($output, $mysql_daemon);
